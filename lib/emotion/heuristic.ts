...
/**
 * Zero-latency, purely lexical emotion + messiness estimate.
 * Runs on every keystroke/paste so the visuals can react instantly,
 * before (or without) the LLM analysis.
 */
...
export function analyzeHeuristic(text: string): EmotionState {
  ...
  const stretchedWords = (lower.match(/([a-z])\1{2,}/g) ?? []).length; // "soooo"
  ...
  // Anger trumps joy when both fire (sarcastic "great, thanks a lot!!!")
